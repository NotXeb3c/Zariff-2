...
impl GestureCursor {
    pub fn init() -> Self {
        match Enigo::new(&Settings::default()) {
            ...
            Err(e) => {
                eprintln!("[Zariff] Gesture cursor control unavailable: {}", e);
                Self(Mutex::new(None))
            }
        }
    }
...
pub fn open_terminal() -> Result<String, String> {
    ...
    Command::new("cmd")
        .args([
            "/C",
            &format!("start powershell -NoProfile -NoExit -Command \"cd '{}'; echo '=== Zariff System Terminal Active ==='\"", cwd.display())
        ])
        .spawn()
        .map_err(|e| e.to_string())?;

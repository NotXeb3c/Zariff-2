<script lang="ts">
  ...
  function descFor(errorType: string): string {
    switch (errorType) {
      case "CircuitBreakerOpenError":
        return "Zariff detected too many consecutive failures in this task and halted further attempts to prevent runaway behavior.";
      case "ActionBudgetExceededError":
      ...
      default:
        return "Zariff halted the task because a budget rule fired.";
    }
  }
</script>

<script lang="ts">
   ...
   * Push-to-talk or continuous listening with wake word "Hey Zariff".
   *
   * Features:
  ...
  const continuousPreferenceKey = "Zariff-continuous-listening-enabled";
  ...
  function initRecognition() {
    ...
    rec.onspeechstart = () => {
      ...
      // correction, preventing Zariff from talking over the user.
    ...
    rec.onresult = (event: any) => {
      ...
      if (final) {
        ...
        if (wakeWordActive) {
          ...
          if (lower.includes("hey Zariff") || lower.includes("hey Zariff,") || lower.startsWith("Zariff")) {
            // Extract command after wake word
            const command = lower
              .replace(/hey Zariff[,]?\s*/i, "")
              .replace(/^Zariff[,]?\s*/i, "")
              .trim();
            if (command) {
              transcript = command;
    ...
    try {
      if (wakeWordActive) {
        ...
        const result = await call<{ status: string; message?: string }>("voice_listener_start", {
          wake_words: ["hey Zariff", "Zariff", "hey pilot"],
        });
...
<style>
  ...
  .pulse-ring {
    ...
    inset: -4px;
    border-radius: 50%;
    border: 2px solid rgba(0, 229, 255, calc(var(--intensity, 0) * 0.7));
    animation: pulse-expand 1s ease-out infinite;
    pointer-events: none;
  }

  .voice-btn.listening .pulse-ring {
    border-color: rgba(255, 46, 166, calc(var(--intensity, 0) * 0.9));
  }
  ...
  @keyframes breathe {
    ...
    50% {
      box-shadow: 0 0 26px rgba(255, 46, 166, 0.8);
    }
  }

<script lang="ts">
  ...
  async function saveHotkey() {
    hotkeyError = "";
    if (!hotkeySupported) {
      hotkeyError = "Open Zariff as the desktop app to register a system-wide shortcut.";
      return;
    }
  ...
  async function applyRootToggle(turningOn: boolean) {
    ...
    if (!synced) {
      rootToast = "Root policy was not changed because the Zariff daemon could not confirm it.";
    ...
    if (turningOn) {
      ...
      rootToast = "Root policy disabled. Root-tier actions are blocked by Zariff.";
    ...
    if (turningOn) {
      askConfirm(
        "⚠️ ENABLE ROOT ACCESS?\n\n" +
          "This unlocks Zariff's full power:\n" +
          "• Admin/sudo shell commands\n" +
          "• System service management\n" +
          "• Protected file modifications\n" +
          "• Registry & disk-level operations\n\n" +
          "Actions requiring elevated privileges will no longer be blocked by Zariff policy.\n" +
  ...
  async function updateAudioInput(e: Event) {
    ...
    speechSaving = false;
    speechToast = synced
      ? "Microphone input saved. Restart Zariff Active to use it."
  ...
  async function testNotification() {
    ...
    try {
      if (!("Notification" in window)) {
        alert("Zariff — Notifications are not supported in this browser.");
        return;
      }
      ...
      alert("Zariff — Test notification triggered! (popups blocked by browser)");
    }
  }
  ...
  <section class="settings-group">
    ...
    <div class="setting-row">
      <div class="setting-info">
        ...
        <span class="setting-desc">Summon Zariff from anywhere (default: Ctrl+Space)</span>
      ...
      <div class="root-status-banner">
        ...
        <div class="root-status-info">
          ...
          <span class="root-status-desc">
            {rootRuntime?.detail ||
              "Root-tier actions are allowed by Zariff policy; checking the daemon's OS privileges."}
          </span>
          {#if rootRuntime?.platform === "win32" && !rootRuntime.process_elevated}
            <span class="root-status-desc"> Windows will show a UAC prompt. Zariff stays running if you cancel. </span>
          {/if}
        </div>
        {#if rootRuntime?.platform === "win32" && !rootRuntime.process_elevated}
      ...
      <div class="setting-info">
        ...
        <span class="security-setting-status">
          {$settings.adaptive_calibration?.voice_wake_word_enabled
            ? "Active · the daemon's Hey Zariff listener can use promoted variants"
  ...
  <section class="settings-group">
    ...
    <div class="setting-row">
      <div class="setting-info">
        ...
        <span class="setting-desc">Audio device used by Zariff Active and local wake-word recognition.</span>
...
<style>
  ...
  .settings-group {
    margin-bottom: 20px;
    background:
      linear-gradient(150deg, rgba(0, 229, 255, 0.04), transparent 40%),
      var(--bg-secondary);

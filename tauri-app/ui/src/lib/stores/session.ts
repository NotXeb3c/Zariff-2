...
async function notifyTaskComplete(payload: Record<string, unknown>) {
  ...
  let title = "Zariff task complete";
  if (status === "error") title = "Zariff task failed";
  else if (status === "partial_failure") title = "Zariff task completed with issues";
  else if (status === "blocked_by_critic") title = "Zariff task blocked by safety review";
  else if (status === "cancelled") title = "Zariff task cancelled";
  else if (dryRun) title = "Zariff dry run complete";
  ...
  try {
    ...
    console.error("[Zariff] notification error:", err);
  }
}
...
function createSession() {
      ...
      case "proactive_suggestion":
        ...
        update((s) => ({
          ...
          proactiveSuggestion: {
            ...
            title: String(p.title ?? "Zariff has a suggestion"),
  ...
  subscribe((s) => {
    ...
    try {
      ...
      console.warn("[Zariff] could not persist chat session", error);
    }
  });
  ...
  async function acceptProactiveSuggestion() {
    ...
    try {
      ...
      if (result.status === "executing") {
        ...
        addSystemMessage("Suggestion accepted. Zariff started it through the guarded task pipeline.");
      } else {

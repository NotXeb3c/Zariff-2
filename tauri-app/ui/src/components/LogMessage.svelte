<!--
  ...
  • Re-uses Zariff's existing clipboard Tauri command for the copy action
-->
...
<script lang="ts">
  ...
  async function handleCopyClick(e: MouseEvent): Promise<void> {
    ...
    try {
      // Use Tauri's clipboard API (matches existing Zariff clipboard_write action)
...
<style>
  ...
  .hlx-message {
    ...
    --color-muted: #5d6498;
    --color-warn: #ffd500;
    --color-text-dim: #8a92c8;
    --color-link: #00e5ff;
    --color-inline-code: #ff2ea6;
    display: flex;
  ...
  .hlx-message--agent {
    ...
    padding-left: 10px;
    box-shadow: inset 3px 0 12px -6px rgba(0, 229, 255, 0.35);
  }
  .hlx-message--user {
    border-left: 2px solid var(--violet);
    padding-left: 10px;
    box-shadow: inset 3px 0 12px -6px rgba(160, 107, 255, 0.3);
  }
  .hlx-message--system {
  ...
  .hlx-content :global(:not(pre) > code) {
    ...
    padding: 1px 5px;
    font-size: 12px;
    color: var(--color-inline-code, #ff2ea6);
    font-family: inherit;
  }
  ...
  .hlx-content :global(.hlx-code-wrapper) {
    ...
    background: #101225; /* Zariff deep-slate code canvas */
  }

        ...
        if (pathname === "/api/tauri_invoke" && req.method === "POST") {
          ...
          req.on("end", () => {
            ...
            if (command === "open_terminal") {
              try {
                execSync(
                  `start powershell -NoProfile -NoExit -Command "cd '${CONFIG_DIR.replace(/\\/g, "/")}'; echo '=== Zariff System Terminal Active ==='"`,
                );
              } catch (e) {
                try {
                  execSync(`start cmd /K echo Zariff System Terminal`);
                } catch (e2) {}
              }
            ...
            if (command === "take_screenshot") {
              ...
              try {
                ...
                try {
                  writeFileSync(shotPath, "Zariff Screenshot Capture Record");
                } catch (e2) {}
              }
